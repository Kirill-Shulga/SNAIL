...
class Convert():
    ...
    @staticmethod
    def Ej_from_Ic(Ic, units_in='nA', units_out='MHz'):
        r'''
        Josephson Junction energy from Josephson Junction critical current.
        Returns in MHz
        ...
        return Convert._convert_num(
            # Plank to go from Joules to Hz
            lambda _Ic: Planck**-1 * (ϕ0)*_Ic,
            Ic, units_in, units_out)
...
##----------------------------------------------------------------------------------------------
    ...
    def ZPF_from_EPR(hfss_freqs, hfss_epr_, hfss_signs, hfss_Ljs,
        ...
        ϕzpfs = epr_to_zpf(hfss_epr, hfss_signs, Ωd, Ej)
